...
function readOrders() {
  for (const division of state.divisions) {
    ...
    state.orders.personnelAssignment[division.id] = Math.max(0, Number(app.querySelector(`[name="personnel-${division.id}"]`).value) || 0);
...
function render() {
  ...
  const convoys = state.transitQueue.map((shipment) => `<div><span class="tag">${esc(shipment.lineId)}</span> → ${esc(shipment.divisionId)} · 弹药 ${number(shipment.cargo.ammo)} / 给养 ${number(shipment.cargo.supply)} · ETA D${shipment.arrivesDay}</div>`).join('') || '<span class="muted">没有在途车队</span>';
  const orderRows = state.divisions.map((division) => `<div class="command-row"><strong>${esc(division.name)}</strong><label>优先级<input name="priority-${division.id}" type="number" min="0" value="${state.orders.divisionPriority[division.id]}"></label><label>最低储备<input name="reserve-${division.id}" type="number" min="0" value="${state.orders.minReserveDays[division.id]}"></label><label>分配人员<input name="personnel-${division.id}" type="number" min="0" value="0"></label><label>回电<input name="truth-${division.id}" type="checkbox" ${state.orders.briefTruth[division.id] ? 'checked' : ''}></label></div>`).join('');
